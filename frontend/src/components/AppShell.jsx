...
// KPI Editor is always reachable. The Results page renders the nav as a top bar
// instead of the left sidebar so the output dashboard gets the full page width.
export default function AppShell({ children }) {
  const router = useRouter();
  const pathname = usePathname();
  const user = getUser();
  const items = [...NAV, KPI_NAV];
  const topNav = pathname === "/app/results";
      ...
      <aside className="shell-side">
        ...
        <nav className="shell-nav">
          {items.map(({ to, label, icon, end }) => (
            <Link key={to} href={to}
              className={`shell-link ${isActive(to, end) ? "active" : ""}`}>
